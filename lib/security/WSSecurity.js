...
const token = wsse({ username: this._username, password: this._password });
 /* TODO: clean redundant code that's no longer needed due to change in what's returned in the header */

  return `<wsse:Security ${(this._actor ? "soap:actor=\"" + this._actor + "\" " : "")} ${(this._mustUnderstand ? "soap:mustUnderstand=\"1\" " : "")}
     xmlns:wsse="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd"
      xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd">
      <wsse:UsernameToken wsu:Id="UsernameToken-${token.getNonce()}">
        ...
        <wsse:Nonce EncodingType="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary">${token.getPasswordDigest()}</wsse:Nonce>
        <wsu:Created>${created}</wsu:Created>
      </wsse:UsernameToken>
      </wsse:Security>`;
};
